...
/**
 * 通用返回对象。与统一异常处理。配套的统一异常处理 com.lwohvye.beans.advice.ResponseResultBodyAdvice类
...
//                Set类型转成List
                ...
                throw new RuntimeException("content类型有误，请求兼容新类型" + ctt.getClass());
        } else {
//            不是page用resultMap字段返回
            this.resultMap = objectMap;
        }
        var tes = objectMap.get("totalElements"); // 总记录数
        if (!Objects.isNull(tes) && tes instanceof Long total)
