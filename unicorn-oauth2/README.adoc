= https://github.com/spring-projects/spring-authorization-server/tree/main/samples[Samples]
...
* 只需一次confirm，不要被sample中的行为误解了，第一次拿了code后，再点击又重新拿了一次，只是apply的scope变了
